...
- ダウンロードページで AI 分析（面談録音の構造化レポート）、一括分析（1回最大30件）、**分析結果を見る**タブでの閲覧、分析結果 zip 一括ダウンロード（分析済みのみ、`*_分析.txt`）、月別推定利用料金の表示
...
ANALYZE_MAX_GLOBAL_DAY=100
ANALYZE_MAX_PER_IP_HOUR=30
ANALYZE_MAX_INPUT_CHARS=80000
USD_JPY_RATE=150
```
...
AI分析はサーバー側で **1日全体100件 / IPあたり1時間30件** のハードリミットがあり、超過時は課金前に遮断されます。一括分析はUI上 **1回最大30件** です。分析結果は D1 にキャッシュされ、再分析時のみ再課金されます。

`/download`（パスコード認証後）では、文字起こしとAI分析を合算した **月別推定利用料金（目安）** を表示します。実際の請求は OpenAI ダッシュボードでご確認ください。料金データは本機能導入以降に蓄積されます。
...
- `/` : アップロード
- `/download` : ダウンロード / 管理 / AI分析 / 月別料金目安 (パスコード: `taichi`)
